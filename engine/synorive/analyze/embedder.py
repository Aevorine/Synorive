...
#: 默认批大小。批越大 padding 浪费越多 —— 一批里最长的那条决定所有条的计算量。
#:
#: 🔴 **2026-08-03 从 16 改回 8。这一行原来和它自己上面那句注释是矛盾的**：
#: 注释写着"实测批 8 最快"，值却是 16，理由写的是"少一半调用开销"。
#: 复测（400 个真实块，intra=1，bge-small-zh-v1.5，块长中位 258 / 最长 1560）：
#:     batch=8   19.95s   20.1 块/秒
#:     batch=16  27.50s   14.5 块/秒   ← 原来的默认值
#:     batch=32  35.72s   11.2 块/秒
#: **批 8 比批 16 快 1.38 倍。** 那点调用开销远远抵不过 padding 浪费。
#:
#: 为什么这一改很值：A7 分段计时实测 **embed 占整个入库耗时的 97.7%**
#: （fingerprint/parse/enrich/chunk/write 五段加起来才 2.3%），
#: 所以嵌入这一段的任何提速几乎 1:1 变成入库吞吐的提速。
#:
#: 另有一条**没有采纳**的：按长度排序后分批能到 1.44 倍，但它会改变向量
#: （最大差 0.03，余弦 0.995）。已验证那不是 attention mask 缺陷，
#: 而是不同批形状下 onnxruntime 选不同内核的浮点噪声 —— 同顺序只换批大小时
#: 结果完全一致（差 0.000000）。收益比批 8 只多一点点，却要引入
#: 排序 + 还原顺序的逻辑，不划算。留在这里备查。
DEFAULT_BATCH = 8
