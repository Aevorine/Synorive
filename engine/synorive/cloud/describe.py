...
DESCRIBE_PROMPT = (
    ...
    # 🔴 内层引号必须用中文全角「」，不能用 ASCII 双引号 ——
    #    这一行原来写的是 `不要用"这张图片展示了"这种套话`，四个 ASCII 双引号
    #    把外层字符串在第三个引号处就截断了，**整个模块从来没编译通过过**。
    #    后果是 C4 图片描述功能一直是死的，而症状只是"开了开关没反应"：
    #    import 失败被上层 try/except 吞掉，日志里只有一行看不出所以然的告警。
