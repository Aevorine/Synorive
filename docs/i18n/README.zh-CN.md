...
[![下载](https://img.shields.io/badge/download-v0.1.11-0F4C8C)](https://github.com/Aevorine/Synorive/releases/latest)
...
| | |
|---|---|
| **Windows 安装包** | [`Synorive-Setup-0.1.11.exe`](https://github.com/Aevorine/Synorive/releases/latest) —— 自带 Python 运行时，**应用内自动更新** |
| **Windows 便携版** | [`Synorive-0.1.11-portable.exe`](https://github.com/Aevorine/Synorive/releases/latest) —— 免安装；这种形式不支持自动更新 |
...
| 🚀 | **开机自启 · 静默进托盘** —— 随系统启动，不弹窗口，只在托盘里待着。等你按下唤起键时，引擎和向量模型已经热好了，不用等冷启动。两个开关都能自己关掉 |
...
> ### ⚠️ 装过 0.1.9 或 0.1.10 的话，请升级
>
> 这两个版本打包进去的 Python 运行时**缺了 `click`** —— uvicorn 的硬依赖。
> 引擎根本起不来：应用能开、托盘图标也有，引擎却在后台无限重启。
> 另外，一旦开启局域网配对，引擎会切到 HTTPS，而桌面端、MCP、CLI 全都还在用 HTTP，
> 三条路一起断。
>
> 两个问题都已在 **0.1.11** 修好，打包自检现在跑在和装机版一模一样的 `sys.path` 上 ——
> 之前那个缺包之所以隐形，正是因为自检验的不是用户将要跑的那个环境。详见 0.1.11 发版说明。
...
npm run version:set 0.1.11  # 一条命令改完四处，别手动改
