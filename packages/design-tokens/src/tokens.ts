...
export const layout = {
  ...
  /**
   * 搜索框在宽屏上的伸缩上限（C1）。
   * 720px 这个死值在 1920px 的窗口上让顶栏搜索框只占 37%，两边各空 600px，
   * 是「内容左右都是空白」最显眼的一处。改成 `min(60vw, 1100px)`：
   * 窄窗口按 60vw 跟着缩，宽窗口停在 1100px 不至于长到一眼扫不完。
   */
  searchBoxFluidMax: 1100,
  /** 内容区左右留白 */
  contentPadding: 24,

  /**
   * 散文型正文（问答、简报、文献正文）的行长上限（C1）。
   *
   * 用 `ch` 不用 px：ch 是当前字体里 "0" 的宽度，字号一变行长跟着变，
   * 用户把系统缩放调到 125% 时**一行的字数不变**，写死 px 的话字变大了、
   * 行还是那么宽，一行就塞进更少的字，排版会莫名其妙地变松。
   *
   * 96ch 的来历（不是拍脑袋）：Times New Roman 的 "0" 宽 0.5em，
   * 所以在小四（16px）语境下 1ch = 8px → 96ch = 768px。
   * 里面真正要读的那段是四号（18.67px）的 `.ans__text`，
   * 768 ÷ 18.67 ≈ 41 个汉字/行 —— 正好卡在 layout.css 注释里那条
   * 「一行超过 40 个汉字，眼睛回行会串行」的线上。
   * 原来写死的 820px 折算是 44 个汉字，其实已经越线了。
   */
  proseMeasure: '96ch',

  /**
   * 非散文视图（列表、卡片墙、时间轴、看板）的整体上限。
   * 这类内容是"扫"不是"读"，行长约束不适用，所以给一个很宽的上限，
   * 主要作用是防止在 4K 超宽屏上拉成一条 3000px 的横线。
   */
  wideMaxWidth: 1440,

  /**
   * 三栏工作台的两条细栏宽度（C1）。
   * 左栏放大纲/步骤/历史，右栏放出处/证据/相关。
   * 左栏比右栏窄：左边是导航（短标题），右边是内容（引文要能读）。
   */
  railLeftWidth: 216,
  railRightWidth: 300,
  ...
  /**
   * 三栏 / 两栏 / 单栏的断点。
   * ⚠️ `@media` **不能吃 CSS 变量**（媒体查询在变量解析之前求值），
   *    所以 layout.css 里写的是同样的字面量。改这里必须同步改那边，
   *    否则表现是"令牌改了但断点没动"，而且不会报任何错。
   */
  railBreakpoint: {
    /** ≥ 这个宽度出右栏（左右两条细栏都在） */
    threeCol: 1600,
  ...
  stage: {
    ...
    /**
     * 舞台顶部留白的**上限**（C1）。
     * 原来写死 180px：1080p 全屏时 0.32×1080−160 = 345 会被夹到 180，
     * 于是首页永远顶着 180px 的空白开头，正是用户说的「上下都为空白」。
     * 96px 仍然保留"比垂直居中略高"的视觉重心，但不再把首屏浪费掉一大截。
     */
    verticalMax: 96,
  },
} as const;
...
export const densityScale = {
  compact: {
    ...
    contentPad: 16,
    /**
     * 卡片墙（.syn-grid / .entitygrid / .brief）里卡片之间的缝（C7）。
     * 和 `gap`（列表行距）分开：一维列表只有上下缝，二维网格的缝
     * 同时是横向的，用同一个值的话紧凑档下卡片会挤成一块砖墙。
     */
    cardGap: 8,
    /** 缩略图 / 预览图的最大边长 */
    thumb: 36,
