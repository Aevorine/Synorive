...
export function WatchPanel({
  ...
  return (
    <section className="syn-watch">
              ...
              <button
                ...
                onClick={() =>
                  void act(`del-${w.id}`, async () => {
                    ...
                    // 🔴 只给真能还原的动作压撤销栈。删库条目那种没有回收站的，
                    // 压进来会变成一个点了以为撤销了、其实什么都没恢复的按钮 ——
                    // 那比没有撤销糟得多。
      ...
      <p className="syn-watch-note">
        订阅只记住「这条 URL 我上次看过没有」，**不按发布时间判** ——
        很多页面根本没有发布时间，而被重新编辑过的旧内容时间戳还会变新。
        每条最多记住 2000 个见过的 URL，超了丢最旧的（订阅要的是"发现新的"，
        三个月前见过什么已经不重要了）。
      </p>
    </section>
  );
